...
import com.sirinsirin.entity.dto.CountInfoDto;
...
import com.sirinsirin.entity.enums.UserSexEnum;
import com.sirinsirin.entity.enums.UserStatusEnum;
import com.sirinsirin.entity.po.UserFocus;
...
public class UserInfoServiceImpl implements UserInfoService {
	...
	public void register(String email, String nickName, String registerPassword) {
		...
		userInfo.setPassword(StringTools.encodeByMd5(registerPassword));	//	将密码加密成md5格式
	...
	public UserInfo getUserDetailInfo(String currentUserId, String userId) {
		...
		//	如果判断成功，说明当前没有用户登录，那么前端默认显示未关注时的按钮样式
	...
	public void updateUserInfo(UserInfo userInfo, TokenUserInfoDto tokenUserInfoDto) {
		...
		//	修改头像时先删除原头像文件
		if(!userInfo.getAvatar().equals(dbInfo.getAvatar())){
			//把删除代码放进redis异步队列中
			executorService.execute(() -> {
				String avatar = dbInfo.getAvatar();
				FileUtils.deleteQuietly(
						new File(appConfig.getProjectFolder() +
								Constants.FILE_FOLDER + avatar));
			});
		}
	...
	@Override
	public void updatePassword(String email, String password, String newPassword, String token) {
		UserInfo userInfo = this.userInfoMapper.selectByEmail(email);
		if(userInfo == null || !userInfo.getPassword().equals(password)){
			throw new BusinessException("账号或密码错误");
		}
