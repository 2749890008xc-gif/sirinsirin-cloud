...
public class FFmpegUtils {
    ...
    public void createImageThumbnail(String filePath) {
        //  extension用于获取上传文件的后缀名
        String extension = FilenameUtils.getExtension(filePath);
        // 获取文件所在目录
        String fullPath = FilenameUtils.getFullPath(filePath);
        // 获取不带后缀的文件名
        String baseName = FilenameUtils.getBaseName(filePath);
        // 拼接缩略图路径：目录 + 基础名 + 缩略图后缀 + 扩展名
        String thumbnailPath = fullPath + baseName + Constants.IMAGE_THUMBNAIL_SUFFIX + extension;
    ...
    public void convertVideo2Ts(File tsFolder, String videoFilePath){
        //  将h264(MP4)编码的文件转成ts格式的文件
        final String CMD_TRANSFER_2TS = "ffmpeg -y -i \"%s\" -vcodec copy -acodec copy -vbsf h264_mp4toannexb \"%s\"";
        //  将ts文件分片，每10秒分一片。“%%”是“%”的转义字符
